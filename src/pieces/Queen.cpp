...
bool Queen::checkValidMove(const Move& move, const GameData& gameData) const {
    ...
    // move must be along one of the axes or diagonal
    if (!isOrthogonalMove(moveView) && !isDiagonalMove(moveView)) {
        return false;
    }
    // path must be empty
